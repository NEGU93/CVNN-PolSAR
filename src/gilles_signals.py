...
def train_monte_carlo(data_name):
    # gets data
    ...
    # Train networks
    monte_carlo_comparison(x_train, y_train, x_test, y_test, filename=os.path.splitext(data_name)[0])
...
if __name__ == '__main__':
    ...
    # train_monte_carlo(data_2chirps_test)
    ...
    da.plot_2_gaussian(d_mean['CVNN acc'], d_std['CVNN acc'], d_mean['RVNN acc'], d_std['RVNN acc'], 'CVNN', 'RVNN',
                       ...
                       filename="./results/accuracy_" + data_name.replace(".mat", ".png"), showfig=False)
    da.plot_2_gaussian(d_mean['CVNN loss'], d_std['CVNN loss'], d_mean['RVNN loss'], d_std['RVNN loss'], 'CVNN', 'RVNN',
                       ...
                       filename="./results/loss_" + data_name.replace(".mat", ".png"), showfig=False)
    da.plot_csv_histogram_matplotlib(path="./results/", filename=data_name.replace(".mat", ".csv"), visualize=True)
